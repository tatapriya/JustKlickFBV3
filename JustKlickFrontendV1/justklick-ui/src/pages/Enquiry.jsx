...
export default function Enquiry() {
  ...
  const handleDeleteNotification = async (id) => {
    try {
      ...
      const updated = myEnquiries.filter((item) => item.id !== id);

      setMyEnquiries(updated);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!listing.id || !listing.categorySlug) {
      alert(
        "Listing details are missing. Please open enquiry from listing page."
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        listing: Number(listing.id),
        listing_name:
          listing.name || listing.itemName || listing.title,
        category: listing.categorySlug,
        name: formData.studentName.trim(),
        email: formData.email.trim(),
        phone: formData.phone.trim(),
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mb-5 rounded-xl bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            <Bell size={20} className="text-[#ef233c]" />
        ...
        <section className="rounded-xl bg-white p-5 shadow-sm">
          <div className="border-b border-gray-100 pb-4">
            ...
            <p className="mt-2 text-sm leading-6 text-gray-500">
              Fill your contact details below. Your enquiry
              will be saved in PostgreSQL and visible in your
              profile.
            </p>
          </div>
